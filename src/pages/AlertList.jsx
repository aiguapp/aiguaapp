...
export default function AlertList({ anomalies = [] }) {
  const getIcon = (type) => {
    switch (type) {
      case 'spike': return <AlertTriangle className="w-5 h-5 text-orange-500" />
      case 'leak':  return <Droplets className="w-5 h-5 text-red-500" />
      case 'drop':  return <TrendingDown className="w-5 h-5 text-yellow-500" />
  ...
  const getTypeLabel = (type) => {
    switch (type) {
      case 'spike': return 'Pico de consumo'
      case 'leak':  return 'Posible fuga'
      case 'drop':  return 'Caída abrupta'
      default:      return 'Anomalía'
    }
  }

  return (
    <div className="bg-white rounded-lg shadow-md p-6 max-w-2xl mx-auto">
      <h2 className="text-xl font-bold text-gray-800 mb-4">Alertas recientes</h2>

      {anomalies.length === 0 ? (
        <div className="text-center py-8 text-gray-500">
          <Droplets className="w-12 h-12 mx-auto mb-2 opacity-30" />
          <p>No se ha detectado ninguna anomalía</p>
        </div>
      ) : (
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span>Desvío: {anomaly.deviation > 0 ? '+' : ''}{anomaly.deviation}%</span>
                  </div>
